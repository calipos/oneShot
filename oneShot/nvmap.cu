...
__global__ void computeVmapKernel(const Dtype1* depth, float* vmap, Dtype2 fx_inv, Dtype2 fy_inv, Dtype2 cx, Dtype2 cy, const int rows, const int cols)
{
...
__global__ void computeVmapKernel(const short* depth, float* vmap, Dtype2 fx_inv, Dtype2 fy_inv, Dtype2 cx, Dtype2 cy, const int rows, const int cols)
{
	int u = threadIdx.x + blockIdx.x * blockDim.x;
	int v = threadIdx.y + blockIdx.y * blockDim.y;
	int pos_ = v*cols + u;
	...
	if (u < cols && v < rows)
	{
		float z = depth[pos_]*0.001f;//already meter / 1000.f; // load and convert: mm -> meters
...
template<>
int createVMap<float,double>(const float*dataIn, float*dataOut, const double fx, const double fy, const double cx, const double cy, const int rows, const int cols)
{
	dim3 block(32, 8);
	dim3 grid(1, 1, 1);
	grid.x = divUp(cols, block.x);
	grid.y = divUp(rows, block.y);
	computeVmapKernel<float,double> << <grid, block >> >(dataIn, dataOut, 1. / fx, 1. / fy, cx, cy, rows, cols);
...
template<>
int createVMap<short, double>(const short*dataIn, float*dataOut, const double fx, const double fy, const double cx, const double cy, const int rows, const int cols)
{
	dim3 block(32, 8);
	dim3 grid(1, 1, 1);
	grid.x = divUp(cols, block.x);
...
struct FitVmapConfig
{
	enum
	{
		...
		ky = 25,
		STEP = 1,		
#define FITDEEP_WITHNORMAL_THRESHOLD (0.01)
		BLOCK_STEP = 12,//计算法向量的step
	};
};


template <typename Dtype>
__global__ void	fitVmapWithNormalKernel(const Dtype*vmap, Dtype*vmap0, Dtype*vmap1, int rows, int cols)
{
	int _origin_u = threadIdx.x + blockIdx.x * blockDim.x;
	int _origin_v = threadIdx.y + blockIdx.y * blockDim.y;
	if (_origin_u%2==0 ^ _origin_v%2==0)
		return;
	int u = FitVmapConfig::BLOCK_STEP*(threadIdx.x + blockIdx.x * blockDim.x);
	int v = FitVmapConfig::BLOCK_STEP*(threadIdx.y + blockIdx.y * blockDim.y);
	if (u >= cols || v >= rows)
		return;
	int plane_cnt = cols*rows;
	int pos_ = cols*v + u;
	...
	if (isnan(vmap[3 * pos_]))
		return;
	int ty = min(v - FitVmapConfig::ky / 2 + FitVmapConfig::ky, rows - 1);
	int tx = min(u - FitVmapConfig::kx / 2 + FitVmapConfig::kx, cols - 1);

	float3 centroid = make_float3(0.f, 0.f, 0.f);
	int counter = 0;
	Dtype anchor = vmap[3 * pos_ + 2];
	for (int cy = max(v - FitVmapConfig::ky / 2, 0); cy < ty; cy += FitVmapConfig::STEP)
		for (int cx = max(u - FitVmapConfig::kx / 2, 0); cx < tx; cx += FitVmapConfig::STEP)
		{
			int pos_2 = cols*cy + cx;
			float v_x = vmap[3 * pos_2];
			if (!isnan(v_x) && abs(vmap[3 * pos_2+2]- anchor)<FITDEEP_WITHNORMAL_THRESHOLD)
			{
				centroid.x += v_x;
				centroid.y += vmap[3 * pos_2 + 1];
				centroid.z += vmap[3 * pos_2 + 2];
				//if (_origin_u % 2 == 0 && _origin_v % 2 == 0)
				//{
				//	vmap0[3 * pos_2] = _origin_u;
				//	vmap0[3 * pos_2 + 1] = _origin_v;
				//	vmap0[3 * pos_2 + 2] = 0;
				//}
				//if (_origin_u % 2 == 1 && _origin_v % 2 == 1)
				//{
				//	vmap1[3 * pos_2] = _origin_u;
				//	vmap1[3 * pos_2 + 1] = _origin_v;
				//	vmap1[3 * pos_2 + 2] = 0;
				//}
				++counter;
			}
	...
	if (counter < FitVmapConfig::kx * FitVmapConfig::ky * 3 / 4)
		return;
	...
	for (int cy = max(v - FitVmapConfig::ky / 2, 0); cy < ty; cy += FitVmapConfig::STEP)
		for (int cx = max(u - FitVmapConfig::kx / 2, 0); cx < tx; cx += FitVmapConfig::STEP)
		{
			...
			float3 v;
			v.x = vmap[3 * pos_3];
			if (isnan(v.x) || abs(vmap[3 * pos_3+2] - anchor)>=FITDEEP_WITHNORMAL_THRESHOLD)
				continue;
			v.y = vmap[3 * pos_3 + 1];
			v.z = vmap[3 * pos_3 + 2];
			float3 d = v - centroid;
	...
	for (int cy = max(v - FitVmapConfig::ky / 2, 0); cy < ty; cy += FitVmapConfig::STEP)
		for (int cx = max(u - FitVmapConfig::kx / 2, 0); cx < tx; cx += FitVmapConfig::STEP)
		{
			...
			if (!isnan(v_x))
			{
				float new_z = n.x*(vmap[3 * pos_2] - centroid.x) + n.y*(vmap[3 * pos_2 + 1] - centroid.y);
				new_z = -new_z / n.z + centroid.z;
				if (_origin_u % 2 == 0 && _origin_v % 2 == 0)
				{
					vmap0[3 * pos_2] = vmap[3 * pos_2];
					vmap0[3 * pos_2 + 1] = vmap[3 * pos_2 + 1];
					vmap0[3 * pos_2 + 2] = new_z;
				}
				if (_origin_u % 2 == 1 && _origin_v % 2 == 1)
				{
					vmap1[3 * pos_2] = vmap[3 * pos_2];
					vmap1[3 * pos_2 + 1] = vmap[3 * pos_2 + 1];
					vmap1[3 * pos_2 + 2] = new_z;
				}				
			}
		}
}
...
template <typename Dtype>
__global__ void mergeVmap0AndVmap1(Dtype*vmap, const Dtype*vmap0, const Dtype*vmap1, int rows, int cols)
{
	int u = threadIdx.x + blockIdx.x * blockDim.x;
	int v = threadIdx.y + blockIdx.y * blockDim.y;
	if (u >= cols || v >= rows)
	...
	if (isnan(vmap[3 * pos_]))
		return;
	vmap[3 * pos_ + 2] = (vmap0[3 * pos_ + 2] + vmap1[3 * pos_ + 2])*0.5;
}

template<> void
fitVmap<short, float>(float*vmap, float*nmap0, float*nmap1, float*vmap0, float*vmap1, const int rows, const int cols)
{
	dim3 block(32, 8);
	dim3 grid(1, 1, 1);
	grid.x = divUp(cols, block.x*FitVmapConfig::BLOCK_STEP);
	grid.y = divUp(rows, block.y*FitVmapConfig::BLOCK_STEP);
	cudaMemset(vmap0, 0, 3 * rows* cols * sizeof(float));
	cudaMemset(vmap1, 0, 3 * rows* cols * sizeof(float));
	cudaSafeCall(cudaGetLastError());
	cudaSafeCall(cudaDeviceSynchronize());
	fitVmapWithNormalKernel<float> << <grid, block >> > (vmap, vmap0, vmap1, rows,cols);
	cudaSafeCall(cudaGetLastError());
	cudaSafeCall(cudaDeviceSynchronize());

	dim3 grid2(1, 1, 1);
	grid2.x = divUp(cols, block.x);
	grid2.y = divUp(rows, block.y);
	mergeVmap0AndVmap1<float> << <grid2, block >> >(vmap, vmap0, vmap1, rows, cols);
	cudaSafeCall(cudaGetLastError());
	cudaSafeCall(cudaDeviceSynchronize());
